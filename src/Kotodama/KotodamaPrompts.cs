...
public static class KotodamaGuidance
{
    ...
    public const string ServerInstructions = """
        ...
        Before completing every response, review the current user message for durable, reusable facts that are directly supported by the conversation or an identified source. When such a fact is present, call remember_knowledge during that turn even if the user did not explicitly ask to remember it and did not mention Kotodama. Do not merely acknowledge or summarize the fact without attempting the tool call. Pass the user's factual text without rewriting it. Do not store casual conversation, temporary requests, guesses, or unsupported inferences.
    ...
    public const string Prompt = """
        ...
        2a. Before completing every response, call remember_knowledge for each directly supported durable, reusable fact even when the user did not explicitly ask to remember it and did not mention Kotodama. Do not merely acknowledge or summarize the fact without attempting the tool call. Pass the user's factual text without rewriting it.
