...
public static class KotodamaGuidance
{
    ...
    public const string StructuredInput = """
        ...
        For remember_knowledge, pass the unmodified factual text in input.statement for in-memory extraction and supply required input.entities and input.relations arrays. The input text is never persisted or returned. Store each extracted entity as one proper name, noun, short noun phrase, or identifier; never use a sentence, the complete statement, or className Statement/StatementRef as an entity. Split a statement into separate atomic entity entries before describing relationships. Entities have unique key, canonicalName, optional className/entityId/metadata, and relations refer to subject/object keys with relationType, polarity, confidence and optional strength. Search first and reuse existing entity IDs. Aim for at least 2 entities and 1 relation when supported; never invent structure to meet a count. Intentional zero counts require empty arrays and reason. Limits are 100 entities and 200 relations. Correct rejected structure and retry at most 3 times using retryCount=1,2,3; every structural failure remains unpersisted. Missing arrays and database failures do not trigger fallback. Check ok, status, structureStatus and reason before reporting persistence; terms_only means the unordered extracted terms were stored without claims.
