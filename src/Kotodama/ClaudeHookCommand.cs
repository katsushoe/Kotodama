...
internal static class ClaudeHookCommand
{
    ...
    private const string PersistReason = """
        Before finishing every completed turn, review the user message and your response for durable, reusable facts even when the user did not explicitly ask to remember them. For Codex, delegate this focused review to the kotodama-curator custom agent when available; otherwise review directly. Use the connected Kotodama MCP tools, search for existing entities and claims first, and store only facts directly supported by the user's factual text or an identified source. Treat assistant-generated text without an identified source as a candidate to discard, not as evidence. When one user statement can be stored through remember_knowledge, pass only that factual statement without rewriting it; never pass or store the raw transcript. If the user explicitly asked to remember, retain, or keep a fact for future use, use Kotodama now rather than built-in memory, loaded instruction files, or acknowledgement alone. Preserve conflicts, include source and temporal metadata only when known, and never store secrets, authentication data, sensitive personal data without explicit approval, guesses, or unsupported inferences. If an equivalent active claim already exists, do not create a duplicate. If there is nothing suitable to store, finish without writing.
        """;
