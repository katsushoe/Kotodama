...
```powershell
Start-Process msiexec.exe -Verb RunAs -Wait -ArgumentList '/i "Kotodama-0.3.1-x64.msi" /norestart'
```

無人Installでは`/qn`を追加します。完了後、Windowsのインストール済みアプリに`Kotodama 0.3.1`が表示されること、`C:\Kotodama\bin\Kotodama.exe`のFile Versionが`0.3.1.0`であること、新しいターミナルで`kotodama`が解決できることを確認します。MSIは`C:\Kotodama\bin`をシステムPATHへ登録し、アンインストール時にその要素を削除します。
...
```powershell
Get-FileHash .\Kotodama-0.3.1-x64.msi -Algorithm SHA256
```
