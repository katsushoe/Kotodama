...
```powershell
Start-Process msiexec.exe -Verb RunAs -Wait -ArgumentList '/i "Kotodama-0.4.0-x64.msi" /norestart'
```

無人Installでは`/qn`を追加します。完了後、Windowsのインストール済みアプリに`Kotodama 0.4.0`が表示されること、`C:\Kotodama\bin\Kotodama.exe`のFile Versionが`0.4.0.0`であること、新しいターミナルで`kotodama`が解決できることを確認します。MSIは`C:\Kotodama\bin`をシステムPATHへ登録し、ログオンユーザーへ`Kotodama MCP Server` Scheduled TaskとCodexの`kotodama` MCP設定を登録します。

Scheduled Taskは`http://127.0.0.1:39280/mcp`でKotodamaを起動します。Codexを再起動するとStreamable HTTP Toolが利用可能になります。アンインストール時はScheduled TaskとCodex設定を削除します。
...
```powershell
Get-FileHash .\Kotodama-0.4.0-x64.msi -Algorithm SHA256
```
