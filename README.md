...
Download [Kotodama-0.17.0-x64.msi](https://github.com/katsushoe/Kotodama/releases/download/v0.17.0/Kotodama-0.17.0-x64.msi), verify its SHA-256, and run it with administrator privileges:

```powershell
Get-FileHash .\Kotodama-0.17.0-x64.msi -Algorithm SHA256
Start-Process msiexec.exe -Verb RunAs -Wait `
  -ArgumentList '/i "Kotodama-0.17.0-x64.msi" /norestart'
```
...
Download [Kotodama-0.17.0-win-x64.zip](https://github.com/katsushoe/Kotodama/releases/download/v0.17.0/Kotodama-0.17.0-win-x64.zip), verify its SHA-256, and extract it to a writable directory:

```powershell
Get-FileHash .\Kotodama-0.17.0-win-x64.zip -Algorithm SHA256
Expand-Archive .\Kotodama-0.17.0-win-x64.zip -DestinationPath C:\Tools\Kotodama
