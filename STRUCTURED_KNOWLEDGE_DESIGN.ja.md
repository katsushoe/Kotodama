...
Claim検索は、正規化テーブルを正本とした派生テーブル`claim_search`を使用します。主語・関係種別・目的語・Claim状態・出典参照を非正規化し、Claimの追加・更新・削除をSQLite triggerで同期します。既存DBは初回初期化時に不足行だけを移行し、正本のEntity、Relation、Claim、Sourceは変更しません。投影は検索専用であり、外部から直接更新しません。
...
- SQL VIEWだけの検索投影: 読み取り時のJOIN処理を削減できないため採用しない。
- `claim_search`を正本とする構造: 出典、矛盾、時制の管理責務が混在するため採用しない。
...
`claim_search`は起動時に不足行を補完します。運用上のバックアップ対象は従来どおりSQLite DB全体で、投影だけを個別に編集・復元しません。テストでは新規Claimの同期、状態更新、既存Claimの補完、検索結果の互換性を確認します。
